...
// Parse & enrich a dns query log.

const moment = require('moment')

const CLASSES = {/* eslint-disable key-spacing */
  '0x0000'    : 'RESERVED',
  '0x00000000': 'RESERVED',
  '0x0001'    : 'INTERNET',
  '0x00000001': 'INTERNET',
...
module.exports = (config, logbus) => {
  function onInput(line) {
    try {
      const m = line.match(/^(\S+)\s+(\S+)\s+([^[]+)\[(\d+)\]:\s+(.+)\s+FAC:(\d+)\s+SEV:(\d+)/u)
      if (!m) {
        return
      }
      const event = {
        ...
        process: m[3].toLowerCase().replace(/_/ug, '-'),
        pid: parseInt(m[4]),
        severity: parseInt(m[7])
      }
      m[5].split(/\s+/u).map(i => i.split(/[:=]/u)).forEach(l => {
        event[l[0]] = l[1]
      })
      if (!event.DIP || !event.DIP.match(/^172/u)) {
        return
      }
      ...
      if (event.IP4 != null && event.IP6 != null) {
        event.R = event.IP4.split(',').concat(event.IP6.split(',')).filter(i => i.trim)
      }
      delete event.IP4
      delete event.IP6
      if (event.R.length === 0) {
