...
// `since` minutes when no last timestamp in `db`.
...
module.exports = (config, logbus) => {
  ...
  if (meta.ts !== undefined) {
    ...
    if (config.since !== undefined) {
      meta.ts.subtract(config.since, 'minutes')
    }
  }
  ...
  function refresh() {
    ...
    execFile('journalctl', args, {}, onJournal)
  }
  ...
  function onJournal(err, stdout, stderr) {
    ...
    try {
      ...
      fs.writeFileSync(db, JSON.stringify(meta))
