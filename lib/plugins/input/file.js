...
module.exports = (config, logbus) => {
  ...
  const paths = {}
  const maxbps = (config.maxMbps || 100) << 20 // eslint-disable-line no-bitwise
  ...
  function onData(data) {
    ...
    logbus.stats({bytes_in: Buffer.byteLength(data)}) // eslint-disable-line camelcase
  }
    ...
    return new Promise(resolve => {
      globs.forEach(pattern => {
        glob.sync(pattern, globopts).forEach(path => {
          ...
          stream.on('error', err => logbus.error(err, `failed to read file: ${path}`))
        })
      })
