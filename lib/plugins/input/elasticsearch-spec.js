...
describe('elasticsearch-input', () => {
  ...
  it('basically works!', async () => {
    ...
    nock(config.endpoint).
      get('/').
      reply(200, {version: {number: '6.8.1'}, cluster_name: 'nocked'}) // eslint-disable-line camelcase
    nock(config.endpoint).
      post('/some-index/_search').
      query(true).
      reply(200, {
        _scroll_id: 'abc123', // eslint-disable-line camelcase
        hits: {hits: [events.e1]},
      })
    nock(config.endpoint).
      post('/_search/scroll').
  ...
  it('supports dynamic index target', async () => {
    ...
    nock(config.endpoint).
      post('/_search/scroll').
      query(true).
      reply(200, {_scroll_id: 'abc123', hits: []}) // eslint-disable-line camelcase
    nock(config.endpoint).
      delete('/_search/scroll').
      reply(200, {})
    await plugin.start()
