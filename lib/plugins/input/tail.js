...
// every `interval` seconds. File metadata (start, age) stored in `db`.
...
Plugin.prototype.start = function() {
  ...
  setInterval(this.stats.bind(this), this.interval * 10000)
}

Plugin.prototype.stats = function() {
  var lines = this.lines
  this.lines = 0
  this.emitStats({lines_in: lines})
}
...
Plugin.prototype.glob = function(err, paths) {
  ...
  for (var path of paths) {
    ...
    var opts = {start: 0, inode: stat.ino}
    if (this.meta[path]) {
    ...
    this.log.info({path: path, start: opts.start}, 'tailing file')
