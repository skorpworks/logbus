...
describe('plugin-sample', () => {
  it('requires either `nth` or `intervalSeconds`', () => {
    ...
    }).toThrow(/undefined config: nth or intervalSeconds/u)
  })
  ...
  it('requires `nth` > 0', () => {
    ...
    }).toThrow(/invalid config: nth must be > 0/u)
  })
  ...
  it('requires `intervalSeconds` > 0', () => {
    ...
    }).toThrow(/invalid config: intervalSeconds must be > 0/u)
  })
  ...
  it('can sample every 10 ms', async () => {
    ...
    await logbus.wait(10, () => {
      plugin.onInput({n: 3})
      expect(logbus.events).toHaveLength(1)
    })
    await logbus.wait(10, () => null)
    await plugin.stop()
    expect(logbus.errors).toHaveLength(0)
    expect(logbus.events).toHaveLength(2)
