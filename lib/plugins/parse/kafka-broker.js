...
module.exports = (config, logbus) => {
  ...
  function asEvent(lines) {
    ...
    if (!lines.length) {
      return null // first event has no prior
    }
    // First "line" is the structured event.
    const event = lines.shift()
    let m = null
    // Parse metadata from []'s.
    m = event.message.match(/^\[([^\]]+)\][:\s]*(.*)/u)
    if (m) {
      event.extra = {message: ''}
      event.message = m[2].trim()
      m[1].replace(/[Pp]artition /u, 'partition=').split(/\s+/u).forEach(s => {
        m = s.match(/(\S+)=([^,\s]+)/u)
        if (m) {
          if (m[1] === 'partition') {
            event[m[1]] = m[2]
    ...
    const partitions = /(\s+partition[:s]*\s+)(\S+)/ugi
    do {
      m = partitions.exec(event.message)
      if (m && !m[2].match(/(as|since)/u)) {
        event.partition = m[2]
    ...
    m = event.message.match(/(.*?)[,\s]*connection id (\S+)(.*)/ui)
    if (m) {
    ...
    if (lines.length) {
      if (lines[0].match(/Exception: /u)) {
        ...
        lines.forEach(function(line) {
          if (line.match(/^(Caused by|\s+at |\s+\.+\s+\d+ more)/u)) {
