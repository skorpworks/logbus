...
module.exports = (config, logbus) => {
  ...
  function onStats(stats) {
    logbus.stats({events_out: stats.txmsgs, bytes_out: tx_bytes})
  }
  ...
  function stop(cb) {
    ...
    if (!logbus.ready) {
      ...
      setTimeout(_.partial(stop, cb).bind(this), 1000)
    }
    else {
      ...
      producer.flush(1000, () => {
        if (tx >= rx) {
          ...
          setTimeout(_.partial(stop, cb), 1000)
        }
      })
    }
  }
