...
// Send to Kafka `topic` every `interval` milliseconds using `compression`
...
function Plugin(config) {
  ...
  if (typeof config.topic === 'function') {
    this.topic = config.topic.bind(config)
  }
  else {
    this.topic = (event) => config.topic
  }
  if (config.format === 'json') {
    this.serialize = (event) => JSON.stringify(event)
  }
  else {
    this.serialize = (event) => event
  }
  this.buffer = []
  var options = {
...
Plugin.prototype.shipEvent = function(event) {
  try {
    this.producer.produce(
      this.topic(event),
      null, // partition
      Buffer.from(this.serialize(event)),
      null, // key
