...
module.exports = (config, logbus) => {
  ...
  function flushBuffer() {
    if (buffer.length) {
      ...
      logbus.stats({events_out: copy.length, bytes_out: sizeof(copy)}) // eslint-disable-line camelcase
    }
  }

  function start() {
    return new Promise(resolve => {
      producer.on('ready', opts => {
        logbus.log.info({opts}, 'connected, ready to send')
        flushBuffer()
