...
Plugin.prototype.onReady = function() {
  this.ready = true
  var that = this
  this.producer.createTopics([this.topic], false, function (err) {
    if (err) {
      that.emitError(err)
    }
  })
  this.log.warn('connected, ready to send')
}
...
Plugin.prototype.stop = function(cb) {
  if (this.ready) {
    if (this.buffer.length > 0) {
      this.log.info('flushing buffered events', {count: this.buffer.length})
      this.run(cb)
    }
    else {
      cb()
    ...
    this.log.warn('still not ready, re-scheduling last run')
