...
Plugin.prototype.run = function() {
  var stats = {
    ...
    ts: moment.utc(),
    errors: this.errors,
    bytes_in: Math.round(this.bytes_in / (1024 * 1024)),
    bytes_out: Math.round(this.bytes_out / (1024 * 1024)),
  ...
  stats.message = util.format('errors[%d] events[in=%d out=%d] lines[in=%d out=%d] mbytes[in=%d out=%d]',
                              stats.errors, stats.events_in, stats.events_out, stats.lines_in, stats.lines_out, stats.bytes_in, stats.bytes_out)
  if (this.enable.memory) {
    var mem = process.memoryUsage()
    stats.heapMB = Math.round(mem.heapUsed / (1024 * 1024))
    stats.rssMB = Math.round(mem.rss / (1024 * 1024))
  }
